...
from astrosource.astrosource_STELLA import TimeSeries
...
#Save the verbose messages in the messages.log file
logging.basicConfig(filename='messages.log', encoding='utf-8', level=logging.DEBUG)
...
@click.option('--period', is_flag=True, type=float, help='Search for periodicity in the data, currently with PDM and Lomb-Scargle methods. This will autoselect a reasonable search range if not provided a range.')
@click.option('--lombscargle', is_flag=True, type=float, help='Initial search for periodicity in the data, only with the Lomb-Scargle method.')
...
@click.option('--referencetime', '-rt', type=float, default=0., help='Reference epoch in modified julian dates')
@click.option('--sigmacomp', type=float, default=0.01, help='Standard deviation of the gaussian distribution used to generate the window function compared to the real data in the Lomb-Scargle method.')
...
@click.option('--starreject', type=float, default=1.0, help='Limit on fraction of stars which can be rejected without image being rejected starting value')
...
@click.option('--noskymap',  is_flag=True, help='Do not use the SkyMapper catalogue for calibration')
@click.option('--noapass',  is_flag=True, help='Do not use the APASS catalogue for calibration')
@click.option('--no2mass',  is_flag=True, help='Do not use the 2MASS catalogue for calibration')
...
def main(full, stars, comparison, variablehunt, notarget, lowestcounts, usescreenedcomps, calibsave, outliererror, outlierstdev, varsearchstdev, varsearchmagwidth, varsearchminimages, ignoreedgefraction, usecompsused, usecompletedcalib, mincompstarstotal, calc, calib, phot, plot, detrend, eebls, period, lombscargle, indir, ra, dec, target_file, format, imgreject, mincompstars, maxcandidatestars, closerejectd, bjd, clean, verbose, periodlower, periodupper, periodtests, referencetime, sigmacomp, thresholdcounts, nopanstarrs, nosdss, noskymap, noapass, no2mass, varsearch, varsearchthresh, starreject, hicounts, lowcounts, colourdetect, linearise, colourterm, colourerror, targetcolour, restrictmagbrightest, restrictmagdimmest, rejectmagbrightest, rejectmagdimmest,targetradius, matchradius):
    ...
    try:
        ...
        if targets is not None:
            ...
            if full or plot:
                ts.plot(detrend=detrend, period=period, lombscargle=lombscargle, eebls=eebls, filesave=True)
    ...
    print(f"Astrosource analysis completed in {toc - tic:0.4f} seconds")
    return
