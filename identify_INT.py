...
def rename_data_file(prihdr, bjd=False):
    ...
    if 'WFFBAND' in prihdr:
        filterOne=(prihdr['WFFBAND'])
    else:
        filters = []
    ...
    if (prihdr['MJD-OBS'] == 'UNKNOWN'):
        ...
        timeobs = '{0:.10f}'.format(prihdr['MJD-OBS']+prihdr['EXPTIME']/2./86400.).replace('.','d')
...
def convert_mjd_bmjd(hdr):
    pointing = SkyCoord(hdr['RA'], hdr['DEC'], unit=(u.hourangle, u.degree), frame='fk5')
    location = EarthLocation.from_geodetic(hdr['LONGITUD'], hdr['LATITUDE'], hdr['HEIGHT'])
    t = Time(hdr['MJD-OBS']+hdr['EXPTIME']/2./86400., format='mjd',scale='utc', location=location)

    tdbholder= (utc_tdb.JDUTC_to_BJDTDB(t, ra=float(pointing.ra.degree), dec=float(pointing.dec.degree), lat=hdr['LONGITUD'], longi=hdr['LATITUDE'], alt=hdr['HEIGHT'], leap_update=True))
