...
def photometry(image,image_file,threshold,min_area,star_scale,min_fwhm,max_fwhm):
...
#Gets in input the fixed extraction radius (in pixel) and the detection threshold 
...
s = float(input("Enter the factor s for the detection threshold s*bkg_rms:\n"))
while s<=0.:
...
photometry(image,image_file=image_file,threshold=s,min_area=2,star_scale=fext,min_fwhm=2,max_fwhm=5)
print('median_g.fits')
image.close()
