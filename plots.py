...
def make_plots(filterCode, paths, photometrydata, fileformat='full'):
    ...
    for j, outputPhot in enumerate(photometrydata):
        r = j + 1
        plt.cla()
        outplotx=asarray(outputPhot)[:,8]
        outploty=asarray(outputPhot)[:,12]
        if  outplotx.size == 0:
...
def phased_plots(paths, filterCode, targets, period, phaseShift):
    ...
    if os.path.exists(paths['parent'] / "LightcurveStats.txt"):
        os.remove(paths['parent'] / "LightcurveStats.txt")
