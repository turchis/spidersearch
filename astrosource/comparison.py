...
def find_comparisons(targets,  parentPath=None, fileList=None, photFileArray=None, photSkyCoord=None, matchRadius=1.45, stdMultiplier=2.5, thresholdCounts=10000000, variabilityMultiplier=2.5, removeTargets=True):
    ...
    outfile, num_comparisons = final_candidate_catalogue(parentPath, photFileArray, fileList, sortStars, thresholdCounts, variabilityMax)
    return outfile, num_comparisons

def final_candidate_catalogue(parentPath, photFileArray, fileList, sortStars, thresholdCounts, variabilityMax):
    ...
    referenceFrame, fileRaDec = find_reference_frame(photFileArray, fileList)
...
def find_reference_frame(photFileArray, fileList):
    fileSizer = 0
    fileindex = 0
...
def catalogue_call(avgCoord, opt, cat_name, targets, closerejectd):
    ...
    if cat_name in ['APASS','PanSTARRS','2MASS']:
...
def find_comparisons_calibrated(targets, paths, filterCode, nopanstarrs=False, nosdss=False, noskymap=False, noapass=False, no2mass=False, colourdetect=False, linearise=False, closerejectd=5.0, max_magerr=0.05, stdMultiplier=2, variabilityMultiplier=2, colourTerm=0.0, colourError=0.0, restrictmagbrightest=-99.9, restrictmagdimmest=99.9, photCoordsFile=None, photFileHolder=None, calibSave=False):
    ...
    FILTERS = {
                ...
                'g' : {'SDSS' : {'filter' : 'gmag', 'error' : 'e_gmag', 'colmatch' : 'rmag', 'colerr' : 'e_rmag', 'colname' : 'g-r', 'colrev' : '0'},
                        'SkyMapper' : {'filter' : 'gPSF', 'error' : 'e_gPSF', 'colmatch' : 'rPSF', 'colerr' : 'e_rPSF', 'colname' : 'g-r', 'colrev' : '0'},
                        'PanSTARRS': {'filter' : 'gmag', 'error' : 'e_gmag', 'colmatch' : 'rmag', 'colerr' : 'e_rmag', 'colname' : 'g-r', 'colrev' : '0'},
                        'APASS' : {'filter' : 'g_mag', 'error' : 'e_g_mag', 'colmatch' : 'r_mag', 'colerr' : 'e_r_mag', 'colname' : 'g-r', 'colrev' : '0'}},
                'r' : {'SDSS' : {'filter' : 'rmag', 'error' : 'e_rmag', 'colmatch' : 'imag', 'colerr' : 'e_imag', 'colname' : 'r-i', 'colrev' : '0'},
                        'SkyMapper' : {'filter' : 'rPSF', 'error' : 'e_rPSF', 'colmatch' : 'iPSF', 'colerr' : 'e_iPSF', 'colname' : 'r-i', 'colrev' : '0'},
                        'PanSTARRS': {'filter' : 'rmag', 'error' : 'e_rmag', 'colmatch' : 'imag', 'colerr' : 'e_imag', 'colname' : 'r-i', 'colrev' : '0'},
                        'APASS' : {'filter' : 'r_mag', 'error' : 'e_r_mag', 'colmatch' : 'i_mag', 'colerr' : 'e_i_mag', 'colname' : 'r-i', 'colrev' : '0'}},
                'i' : {'SDSS' : {'filter' : 'imag', 'error' : 'e_imag', 'colmatch' : 'rmag', 'colerr' : 'e_rmag', 'colname' : 'r-i', 'colrev' : '1'},
                        'SkyMapper' : {'filter' : 'iPSF', 'error' : 'e_iPSF', 'colmatch' : 'rPSF', 'colerr' : 'e_rPSF', 'colname' : 'r-i', 'colrev' : '1'},
                        'PanSTARRS': {'filter' : 'imag', 'error' : 'e_imag', 'colmatch' : 'rmag', 'colerr' : 'e_rmag', 'colname' : 'r-i', 'colrev' : '1'},
                        'APASS' : {'filter' : 'i_mag', 'error' : 'e_i_mag', 'colmatch' : 'r_mag', 'colerr' : 'e_r_mag', 'colname' : 'r-i', 'colrev' : '1'}},
                'z' : {'PanSTARRS': {'filter' : 'zmag', 'error' : 'e_zmag', 'colmatch' : 'rmag', 'colerr' : 'e_rmag', 'colname' : 'r-zs', 'colrev' : '1'},
                        'SkyMapper' : {'filter' : 'zPSF', 'error' : 'e_zPSF', 'colmatch' : 'rPSF', 'colerr' : 'e_rPSF', 'colname' : 'r-zs', 'colrev' : '1'},
                        'SDSS' : {'filter' : 'zmag', 'error' : 'e_zmag', 'colmatch' : 'rmag', 'colerr' : 'e_rmag', 'colname' : 'r-zs', 'colrev' : '1'}},
                'J' : {'2MASS': {'filter' : 'Jmag', 'error' : 'e_Jmag', 'colmatch' : 'Hmag', 'colerr' : 'e_Hmag', 'colname' : 'J-H', 'colrev' : '0'}}
                }
    ...
    for cat_name, opt in catalogues.items():
        try:
            if coords ==[]: #SALERT - Do not search if a suitable catalogue has already been found
                ...
                elif cat_name == 'SkyMapper' and noskymap==True:
                    logger.info("Skipping SkyMapper")
                elif cat_name == 'APASS' and noapass==True:
                    logger.info("Skipping APASS")
                #For IR calibrated sources
                elif cat_name == '2MASS' and no2mass==True:
                    logger.info("Skipping 2MASS")
                else:
                    print(cat_name, opt)
                    ...
                    logger.info("Setting catalogueOut")
    ...
    for file in fileList:
        ...
        for q in range(len(idx)):
            #if d2d[q] < max_sep:
            if colrev == 1:
                photFile[idx[q],10]=coords.colmatch[q]-coords.mag[q]
            else:
                photFile[idx[q],10]=coords.mag[q]-coords.colmatch[q]
            photFile[idx[q],11]=pow(pow(coords.colerr[q],2)+pow(coords.emag[q],2),0.5)
            photFile[idx[q],12]=1                
        ...
        photFile[:,10]=np.nan_to_num(photFile[:,10],nan=np.nanmedian(photFile[:,10]))
        photFile[:,11]=np.nan_to_num(photFile[:,11],nan=np.nanmedian(photFile[:,11]))
        ...
        notNanTemp[:,4]=-2.5*log10(notNanTemp[:,4]) -(colourTerm*notNanTemp[:,9])
        
        
        NanTemp=photFile[~np.isnan(photFile).any(axis=1)] #rows where no value is nan
        NanTemp[:,4]=-2.5*log10(NanTemp[:,4])
        NanTemp[:,11]=2 # 2 means that there was no colour to use to embed the colour.
        ...
        if calibStands.size == 13 and calibStands.shape[0]== 13:
            for q in range(len(calibStands[:,0])):        
                idx,d2d,_=calibCoord.match_to_catalog_sky(photCoords)
                if photFile[idx,12] != 0:
                    tempDiff.append(calibStands[3]-(photFile[idx,4]))
                    calibOut.append([calibStands[3],calibStands[4],photFile[idx,4],photFile[idx,5],calibStands[3]-(photFile[idx,4]),0,photFile[idx,10],photFile[idx,0],photFile[idx,1]])
        else:
            ...
            for q in range(len(idx)):  
                if photFile[idx[q],12] != 0:
                    tempDiff.append(calibStands[q,3]-(photFile[idx[q],4]))
                    calibOut.append([calibStands[q,3],calibStands[q,4],photFile[idx[q],4],photFile[idx[q],5],calibStands[q,3]-(photFile[idx[q],4]),0,photFile[idx[q],10],photFile[idx[q],0],photFile[idx[q],1]])
        ...
        if colourdetect == True and (not np.all(np.isnan(photFile[:,9]))):
            # Colour Term Reference plot
            plt.cla()
            fig = plt.gcf()
            outplotx=calibOut[:,6]
