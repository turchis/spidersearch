...
def extract_photometry(infile, parentPath, outfile=None, bjd=False, ignoreedgefraction=0.05, lowestcounts=0):
...
def convert_mjd_bmjd(hdr):
    pointing = SkyCoord(hdr['RA'], hdr['DEC'], unit=(u.hourangle, u.degree), frame='icrs')
...
def find_stars(targets, paths, fileList, photCoords=None, photFileHolder=None, mincompstars=0.1, mincompstarstotal=-99, starreject=0.1 , acceptDistance=1.0, lowcounts=0, hicounts=3000000, imageFracReject=0.0,  rejectStart=3, maxcandidatestars=10000):
    ...
    for file in fileList:
        ...
        if(bool(re.search("median+", file))):
            ...
            logger.debug("NUMBER OF STARS {} - IN THE FILE {}".format(referenceFrame.shape[0], file))
        counter=counter+1
    ...
    logger.debug("NUMBER OF STARS AFTER THE FIRST CUT")
    logger.debug(referenceFrame.shape[0])
