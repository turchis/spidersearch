...
def rename_data_file(prihdr, bjd=False):
    ...
    expTime=(str(prihdr['EXPT']).replace('.','d'))
    ...
    if (prihdr['JD-OBS'] == 'UNKNOWN'):
        timeobs = 'UNKNOWN'
    elif bjd:
        timeobs = convert_mjd_bmjd(prihdr)
    else:
        timeobs = '{0:.10f}'.format(prihdr['JD-OBS']+prihdr['EXPT']/2./86400.).replace('.','d')
...
def convert_mjd_bmjd(hdr):
    pointing = SkyCoord(hdr['RAWCS'], hdr['DEWCS'], unit=(u.hourangle, u.degree), frame='fk5')
    location = EarthLocation.from_geodetic(hdr['SITELONG'], hdr['SITELAT'], hdr['TELALT'])
    t = Time(hdr['JD-OBS']+hdr['EXPT']/2./86400., format='jd',scale='utc', location=location)

    tdbholder= (utc_tdb.JDUTC_to_BJDTDB(t, ra=float(pointing.ra.degree), dec=float(pointing.dec.degree), lat=hdr['SITELAT'], longi=hdr['SITELONG'], alt=hdr['TELALT'], leap_update=True))
