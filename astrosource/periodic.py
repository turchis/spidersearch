...
def aov_periodfind(times,
    ...
    if len(stimes) > 9 and len(smags) > 9 and len(serrs) > 9:
        ...
        savetxt(periodPath / tempfile, np.column_stack(periods, lsp), delimiter=",", fmt='%0.12f')
        ...
        savetxt(periodPath / tempfile, np.column_stack(((times/finperiods[bestperiodind])%1, times, mags)), delimiter=",", fmt='%0.12f')
...
def getPhases(julian_dates, fluxes, period, t_0):

    phases = []

    for jd in julian_dates:
        phases.append(((jd-t_0) / period) % 1)
...
def phase_dispersion_minimization(varData, periodsteps, minperiod, maxperiod, numBins, t_0, periodPath, variableName):
    ...
    for r in range(periodsteps):
        ...
        (sortedPhases, sortedNormalizedFluxes) = getPhases(julian_dates, normalizedFluxes, periodguess, t_0)
    ...
    periodTrialMatrix=[]
    for r in range(periodsteps):
        periodTrialMatrix.append([periodguess_array[r],stdev_results[r]])
    ...
    if np.isnan(pdm["stdev_results"][0]) or pdm["stdev_results"][0] == 0.0:
        ...
        np.savetxt(periodPath / f"{variableName}_Trials.csv", periodTrialMatrix, delimiter=",", fmt='%0.12f')
...
def LombScargleMultiterm(infile, t, m, d, periodlower=0.2, periodupper=2.5, nterms=1, multisearch=False, samples=5,
                         disablelightcurve=False, periodPath=False, variableName="NoName", periodsteps=10000, t_0=0., sigmacomp=0.01):
    #print(
    ...
    #Calculate the Lomb-Scargle periodogram values for a function gaussian distributed, with mu=mean of our data and sigma=0.01 mag, to compare the peaks we find in the periodogram of our time series
    testm = np.random.normal(np.mean(m), sigmacomp, len(t))
    #testm = np.full(len(t), np.mean(m))
    testls = LombScargle(t, testm, nterms=nterms, fit_mean=True)
    try:
        
        #freq, power = ls.autopower(samples_per_peak=samples, minimum_frequency=1 / periodupper,
        #                           maximum_frequency=1 / periodlower)
        testpower = testls.power(freq)
    except:
        print ("Test Lomb Scargle failed.") # Need to hunt down a very rare but existant memory problem
    #    return (0)
    ...
    # Output likelihoods csv for real data and gaussian lightcurve (periods, power, gausspower)
    if 'power' in locals() and 'testpower' in locals():
        tempfile=str(f"{variableName}_LombScargle_N" + str(nterms) + "_Likelihood.csv")
        savetxt(periodPath / tempfile, np.column_stack((1 / freq, power, testpower)), delimiter=",", fmt='%0.12f')
    ...
    if not disablelightcurve:
        ...
        # Output lightcurves csv for real data and gaussian data (t, phase, gaussm, m, d, best_period)
        tempfile=str(f"{variableName}_LombScargle_N" + str(nterms) + "_Lightcurve.csv")
        savetxt(periodPath / tempfile, np.column_stack((t, ((t-t_0) * best_freq) % 1, testm, m, d, np.full(len(t),1/best_freq))), delimiter=",", fmt='%0.12f')
...
def plot_with_period_LS(paths, filterCode, numBins = 10, minperiod=0.2, maxperiod=1.2, periodsteps=10000, t_0=0., sigmacomp=0.01):
    ...
    for file in fileList:
        dateList.append(file.split('_')[2].replace('d','.'))
    dateList=np.asarray(dateList,dtype=float)
    if maxperiod > (np.max(dateList)-np.min(dateList)):
        maxperiod = (np.max(dateList)-np.min(dateList))    
    ...
    for file in fileList:
        ...
        #if calibFile.exists():
        #    if (calibData.size > 3):
        #        pdm=phase_dispersion_minimization(calibData, periodsteps, minperiod, maxperiod, numBins, t_0, periodPath, variableName)
        #else:
        #    pdm=phase_dispersion_minimization(varData, periodsteps, minperiod, maxperiod, numBins, t_0, periodPath, variableName)
        ...
        #        tempPeriodCatOut=[]
        #        for g in range(len(calibData[:,0])):
        #            tempPeriodCatOut.append([((calibData[g,0]-t_0)/(pdm["stdev_minperiod"])) % 1, calibData[g,1], calibData[g,2]])
        #        tempPeriodCatOut=asarray(tempPeriodCatOut)
        ...
        #    tempPeriodCatOut=[]
        #    for g in range(len(varData[:,0])):
        #        tempPeriodCatOut.append([((varData[g,0]-t_0)/(pdm["stdev_minperiod"])) % 1, varData[g,1], varData[g,2]])
        #    tempPeriodCatOut=asarray(tempPeriodCatOut)
            ...
            if calibFile.exists():
                ...
                if (len(calibData)>3):
                    lscargoutput = LombScargleMultiterm('periodifile', (calibData[:, 0]), (calibData[:, 1]), (calibData[:, 2]),
                                                        ...
                                                        disablelightcurve=False, periodPath=periodPath, variableName=variableName, periodsteps=periodsteps, t_0=t_0, sigmacomp=sigmacomp)
                    bestperiods.append(lscargoutput)
    ...
    if 'bestperiods' in locals() and len(bestperiods)!=0:
        return bestperiods[0]
    else:
        return 0.0
...
def plot_with_period(paths, filterCode, numBins = 10, minperiod=0.2, maxperiod=1.2, periodsteps=10000, t_0=0., sigmacomp=0.01):
    ...
    fileList = paths['outcatPath'].glob('*_diffExcel.csv')
    if os.path.exists(paths['parent'] / "periodEstimates.txt"):
        os.remove(paths['parent'] / "periodEstimates.txt")
    with open(paths['parent'] / "periodEstimates.txt", "w") as f:
        f.write("Period Estimates \n\n")
    ...
    for file in fileList:
        ...
        if 'pdm' in locals():
            if np.isnan(pdm["stdev_results"][0]) or pdm["stdev_results"][0] == 0.0 or (len(varData) < 4):
                logger.info("No PDM results due to lack of datapoint coverage")
            else:
                logger.debug("PDM Method Estimate (days): "+ str(pdm["stdev_minperiod"]))
                phaseTest=(varData[:,0] / (pdm["stdev_minperiod"])) % 1
                ...
                if calibFile.exists():
                    ...
                    if (len(calibData)>3):
                        ...
                        for g in range(len(calibData[:,0])):
                            tempPeriodCatOut.append([((calibData[g,0]-t_0)/(pdm["stdev_minperiod"])) % 1, calibData[g,1], calibData[g,2]])
                        tempPeriodCatOut=asarray(tempPeriodCatOut)
                        savetxt(periodPath / f"{variableName}_PDM_PhasedCalibMags.csv", tempPeriodCatOut, delimiter=",", fmt='%0.12f')
                ...
                if (len(varData)>3):
                    ...
                    for g in range(len(phaseTest)):
                        tempPeriodCatOut.append([phaseTest[g],varData[g,1]])
                    tempPeriodCatOut=asarray(tempPeriodCatOut)
                    savetxt(periodPath / f"{variableName}_PDMTrial.csv", tempPeriodCatOut, delimiter=",", fmt='%0.12f')

                    tempPeriodCatOut=[]
                    for g in range(len(varData[:,0])):
                        tempPeriodCatOut.append([((calibData[g,0]-t_0)/(pdm["stdev_minperiod"])) % 1, varData[g,1], varData[g,2]])
                    tempPeriodCatOut=asarray(tempPeriodCatOut)
                    savetxt(periodPath / f"{variableName}_PDM_PhaseddiffMags.csv", tempPeriodCatOut, delimiter=",", fmt='%0.12f')
