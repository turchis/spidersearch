...
def get_total_counts(photFileArray, compFile, loopLength, photCoords):
    ...
    for photFile in photFileArray:
        ...
        for j in range(loopLength):
            ...
            allCountsErr = add(allCountsErr, photFile[idx][5]**2)
            allCountsErr = pow(allCountsErr, 0.5)
...
def find_variable_stars(targets, matchRadius, errorReject=0.05, parentPath=None, varsearchthresh=0, varsearchstdev=2.0, varsearchmagwidth=0.25, varsearchminimages=0.0, photCoords=None, photFileHolder=None, fileList=None):
    '''
    ...
    for photFile in photFileArray:
        ...
        if(bool(re.search("median+", fileList[fileindex-1]))):
            referenceFrame = photFile
            fileSizer = photFile.size
            referenceindex = fileindex
    print(referenceFrame)
    # Delete the reference (median) image from the list to compute the real variability of the source
    photFileArray = delete(photFileArray, referenceindex-1, axis=0)
    ...
    print("TOTAL NUMBER OF STARS IN REFERENCE FRAME: {}".format(targetFile.shape[0]))
    ...
    for target in targetFile:
        ...
        r=0
        for photFile in photFileArray:
            #logger.debug("photFile: {}".format(photFile))
            #compList=[]
            fileRaDec = photFileCoords[r]
            r=r+1
            idx, d2d, _ = varCoord.match_to_catalog_sky(fileRaDec)
            #logger.debug("Index of the source: {}".format(idx))
            #logger.debug("Shape of fileRaDec: {}".format(fileRaDec.shape))
            #logger.debug("Shape of photFile: {}".format(photFile.shape))
        ...
        if (diffMagHolder.shape[0] > 1):
            outputVariableHolder.append( [target[0],target[1],median(diffMagHolder), std(diffMagHolder), diffMagHolder.shape[0]])
    
    logger.debug("TOTAL NUMBER OF STARS IN starVariability.csv: {}".format(len(outputVariableHolder)))    
    print("TOTAL NUMBER OF STARS IN starVariability.csv: {}".format(len(outputVariableHolder)))
...
def photometric_calculations(targets, paths, targetRadius, colourterm, colourerror, colourdetect, linearise, targetcolour, rejectmagbrightest, rejectmagdimmest, calibCompFile, errorReject=0.1, filesave=True, outliererror=4, outlierstdev=4, photCoordsFile=None, photFileArray=None, fileList=None):
    ...
    if len(targets)== 4 and targets.size == 4:
        ...
        logger.debug("LENGTH OF THE TARGETS {}".format(loopLength))
    ...
    for q in range(loopLength):
        ...
        for imgs, photFile in enumerate(photFileArray):
            #logger.debug("Image {}".format(imgs+1))
            sys.stdout.write('.')
            #compList=[]
            #fileRaDec = SkyCoord(ra=photFile[:,0]*degree, dec=photFile[:,1]*degree)
            fileRaDec = photCoordsFile[imgs]
            idx, d2d, _ = varCoord.match_to_catalog_sky(fileRaDec)
            #logger.debug("Distance to the catalogue source {}".format(d2d.arcsecond))
            starRejected=0
            if (less(d2d.arcsecond, targetRadius)):
                magErrVar = 1.0857 * (photFile[idx][5]/photFile[idx][4])
                if magErrVar < errorReject:
                    #logger.debug("Source detected in image {}".format(imgs+1))
            ...
            if ( starRejected == 1):
                    ...
                    #templist is a temporary holder of the resulting file.
                    tempList=photFileArray[imgs][idx,0:8]
                    googFile = Path(fileList[imgs]).name
                    tempList=append(tempList, float(googFile.split("_")[2].replace("d",".")))
                    tempList=append(tempList, float(googFile.split("_")[4].replace("a",".")))
                    tempList=append(tempList, allCountsArray[allcountscount][0])
        ...
        for j in range(asarray(outputPhot).shape[0]):
            if isnan(outputPhot[j][13]):
                imageReject.append(j)
        #print (imageReject)
        outputPhot=delete(outputPhot, imageReject, axis=0)

        if np.all(np.isnan(outputPhot)):
            logger.debug("Rejecting Variable {}: not detected in any image".format(q+1))
        else:
            ...
            #outputPhot=asarray
            ## REMOVE MAJOR OUTLIERS FROM CONSIDERATION
            stdVar=nanstd((outputPhot)[:,12])
            avgVar=nanmean((outputPhot)[:,12])
            starReject=[]
            stdevReject=0
            ...
            while True:
                starReject=[]
                for j in range(asarray(outputPhot).shape[0]):
                    if outputPhot[j][12] > avgVar+(4*stdVar) or outputPhot[j][12] < avgVar-(4*stdVar) :
                        starReject.append(j)
            ...
            while True:
                errorsArray=[]
                for j in range(asarray(outputPhot).shape[0]):
                    errorsArray.append(outputPhot[j][13])
                ...
                starReject=[]
                for j in range(asarray(outputPhot).shape[0]):
                    if outputPhot[j][13] > avgErrors+(4*stdErrors):
                        starReject.append(j)
                        starErrorRejCount=starErrorRejCount+1
                if starReject != []:
                    outputPhot=delete(outputPhot, starReject, axis=0)
            ...
            if single_value:
                ...
                for j in range(len(ensembleMag)):
                    ensMag=ensMag+(pow(10,-ensembleMag[j]*0.4))
            ...
            if (targetcolour == -99.0):
                for i in range(outputPhot.shape[0]):
                    if (ensembleMag+outputPhot[i][12]) > rejectmagbrightest and (ensembleMag+outputPhot[i][12]) < rejectmagdimmest:
                        outputPhot[i][calibIndex-1]=ensembleMag+outputPhot[i][12] # Calibrated Magnitude SKIPPING colour term
                        ...
                        outputPhot[i][calibIndex]=pow(pow(outputPhot[i][13],2)+pow(ensembleMagError,2),0.5) # Calibrated Magnitude Error. NEEDS ADDING in calibration error. NEEDS ADDING IN COLOUR ERROR
                    else:
                ...
                for i in range(outputPhot.shape[0]):
                    if (ensembleMag+outputPhot[i][12]-(colourterm * targetcolour)) > rejectmagbrightest and (ensembleMag+outputPhot[i][12]-(colourterm * targetcolour)) < rejectmagdimmest:
                        outputPhot[i][calibIndex-1]=ensembleMag+outputPhot[i][12] - (colourterm * targetcolour) # Calibrated Magnitude incorporating colour term
                        #outputPhot[i][calibIndex]=pow(pow(outputPhot[i][11],2)+pow(errCalib,2),0.5) # Calibrated Magnitude Error. NEEDS ADDING in calibration error. NEEDS ADDING IN COLOUR ERROR
                        outputPhot[i][calibIndex]=pow(pow(outputPhot[i][13],2)+pow(ensembleMagError,2),0.5) # Calibrated Magnitude Error. NEEDS ADDING in calibration error. NEEDS ADDING IN COLOUR ERROR
                    else:
                        outputPhot[i][calibIndex-1]=np.nan
            ...
            logger.info("Outputting files Variable " + str(q+1))
            for mode in ['diff','calib']:
                if mode == 'calib':
                    if not (paths['parent'] / 'calibCompsUsed.csv').exists():
                    ...
                    magColumn=outputPhot[:,12]
                    magerrColumn=outputPhot[:,13]

                outputPeransoCalib = [x for x in zip(outputPhot[:,8],magColumn,magerrColumn)]
                ...
                if not outputPeransoCalib.size == 0:
                    ...
                    #savetxt(paths['outcatPath'] / f'V{str(q+1)}_{mode}Peranso.txt', outputPeransoCalib, delimiter=" ", fmt='%0.8f')
                    savetxt(paths['outcatPath'] / f'V{str(q+1)}_{mode}Excel.csv', outputPeransoCalib, delimiter=",", fmt='%0.8f')

                    #output for EXOTIC modelling
                    outputEXOTICCalib = [x for x in zip(outputPhot[:,8],magColumn,magerrColumn,outputPhot[:,9])]

                    outputEXOTICCalib=asarray(outputEXOTICCalib)
                    exoMedian=median(outputEXOTICCalib[:,1])

                    for j in range (outputEXOTICCalib.shape[0]):
                        outputEXOTICCalib[j][1]=(1-pow(10,((outputEXOTICCalib[j][1]-exoMedian)/2.5)))+1
                        outputEXOTICCalib[j][2]=(outputEXOTICCalib[j][2]/1.0857)*outputEXOTICCalib[j][1]
                    ...
                    savetxt(paths['outcatPath'] / f'V{str(q+1)}_{mode}EXOTIC.csv', outputEXOTICCalib, delimiter=",", fmt='%0.8f')

                    # Output Differential astroImageJ file
                    outputaijCalib = [x for x in zip(outputPhot[:,8]-2450000.0,magColumn,magerrColumn)]
                    outputaijCalib = asarray(outputaijCalib)
                    outputaijCalib = outputaijCalib[~isnan(outputaijCalib).any(axis=1)]
    
                    savetxt(paths['outcatPath'] / f'V{str(q+1)}_{mode}AIJ.txt', outputaijCalib, delimiter=" ", fmt='%0.8f')
                    savetxt(paths['outcatPath'] / f'V{str(q+1)}_{mode}AIJ.csv', outputaijCalib, delimiter=",", fmt='%0.8f')
        ...
        if (targetcolour == -99.0):
            for i in range(outputPhot.shape[0]):
                if (ensembleMag+outputPhot[i][12]) > rejectmagbrightest and (ensembleMag+outputPhot[i][12]) < rejectmagdimmest:
                    ...
                    #outputPhot[i][calibIndex]=pow(pow(outputPhot[i][11],2)+pow(errCalib,2),0.5) # Calibrated Magnitude Error. NEEDS ADDING in calibration error. NEEDS ADDING IN COLOUR ERROR
                    #outputPhot[i][calibIndex]=pow(pow(outputPhot[i][13],2)+pow(ensembleMagError,2),0.5) # Calibrated Magnitude Error. NEEDS ADDING in calibration error. NEEDS ADDING IN COLOUR ERROR
                    outputPhot[i][calibIndex]=outputPhot[i][13] # Calibrated Magnitude Error. NO NEEDS ADDING in calibration error.
                else:
                    outputPhot[i][calibIndex-1]=np.nan
                    outputPhot[i][calibIndex]=np.nan
