...
from astrosource.identify_INT import find_stars, gather_files
from astrosource.periodic import plot_with_period
from astrosource.periodic import plot_with_period_LS
...
class TimeSeries:
    def __init__(self, targets, indir, **kwargs):
        ...
        self.referencetime = kwargs.get('referencetime', -99.9)
        self.periodtests = kwargs.get('periodtests', -99)
        self.sigmacomp = kwargs.get('sigmacomp', 0.01)
        ...
        self.noskymap = kwargs.get('noskymap', False)
        self.noapass = kwargs.get('noapass', False)
        self.no2mass = kwargs.get('no2mass', False)
    ...
    def analyse(self, calib=True, usescreenedcomps=False, usecompsused=False, usecompletedcalib=False):
        ...
        if usecompletedcalib == False:
            ...
            if calib and self.filtercode in ['B', 'V', 'up', 'gp', 'rp', 'ip', 'zs', 'g', 'r', 'i', 'z', 'CV', 'J']:
                try:
                    ...
                    self.colourterm, self.colourerror, self.calibcompsused = find_comparisons_calibrated(targets=self.targets,
                                                                                    ...
                                                                                    nosdss=self.nosdss,
                                                                                    noskymap=self.noskymap,
                                                                                    noapass=self.noapass,
                                                                                    no2mass=self.no2mass,
    ...
    def photometry(self, filesave=False, targets=None):
        ...
        self.data = photometric_calculations(targets=self.targets, paths=self.paths, targetRadius=self.targetradius, colourterm=self.colourterm,colourerror=self.colourerror,colourdetect=self.colourdetect,linearise=self.linearise,targetcolour=self.targetcolour,rejectmagbrightest=self.rejectmagbrightest,rejectmagdimmest=self.rejectmagdimmest, calibCompFile=self.calibcompsused, filesave=filesave, outliererror=self.outliererror, outlierstdev=self.outlierstdev,photCoordsFile=self.photCoords,photFileArray=self.photFileHolder, fileList=self.usedimages)
        self.output(mode='diff', data=self.data)
        self.output(mode='calib', data=self.data)
            ...
            #self.output(mode='calib', data=data)
        #else:
            #self.data = data

    def plot(self, detrend=False, eebls=False, period=False, lombscargle=False, phaseShift=0.0, filesave=False):
        ...
        if period:
            if lombscargle:
                self.period = plot_with_period_LS(filterCode=self.filtercode, paths=self.paths, minperiod=self.periodlower, maxperiod=self.periodupper, periodsteps=self.periodtests, t_0=self.referencetime, sigmacomp=self.sigmacomp)
                if self.calibrated and self.period!=0:
                    phased_plots(filterCode=self.filtercode, paths=self.paths, targets=self.targets, period=self.period, phaseShift=phaseShift)
            else:
                self.period = plot_with_period(filterCode=self.filtercode, paths=self.paths, minperiod=self.periodlower, maxperiod=self.periodupper, periodsteps=self.periodtests, t_0=self.referencetime, sigmacomp=self.sigmacomp)
                if self.calibrated and self.period!=0:
                    phased_plots(filterCode=self.filtercode, paths=self.paths, targets=self.targets, period=self.period, phaseShift=phaseShift)
        if eebls:
