...
def rename_data_file(prihdr, bjd=False):
    ...
    filterOne=prihdr['FAFLTNM'][0]

    expTime=(str(prihdr['EXPTIME']).replace('.','d'))
    dateObs=(prihdr['DATE-AVG'].replace('-','d').replace(':','d').replace('.','d'))
    airMass=(str(prihdr['AIRMASS']).replace('.','a'))
    instruMe=(prihdr['INSTRUME']).replace(' ','').replace('/','').replace('-','')



    if (prihdr['DATE-AVG'] == 'UNKNOWN'):
        timeobs = 'UNKNOWN'
    elif bjd:
        timeobs = convert_mjd_bmjd(prihdr)
    else:
        timeobs = '{0:.10f}'.format(Time(prihdr['DATE-AVG'], format='isot', scale='utc').mjd).replace('.','d')
...
def extract_photometry(infile, parentPath, outfile=None, bjd=False, ignoreedgefraction=0.0, lowestcounts=0):
    ...
    with fits.open(infile) as hdulist:
        ...
        if not outfile:
            outfile = rename_data_file(hdulist[0].header, bjd=bjd)
        ...
        xpixel = data['x']
        ypixel = data['y']
        ra, dec = w.all_pix2world(xpixel, ypixel, 1)
        counts = data['flux']
        ...
        photFile = transpose([ra, dec, xpixel, ypixel, counts, countserr, fwhm, flag, zerosphot, zerosphot])
...
def convert_photometry_files(filelist, ignoreedgefraction=0.0, lowestcounts=0):
    new_files = []
    photFileHolder=[]
    photSkyCoord=[]
    for fn in filelist:
...
def convert_mjd_bmjd(hdr):
    pointing = SkyCoord(hdr['RA'], hdr['DEC'], unit=(u.degree, u.degree), frame='fk5')
    location = EarthLocation.from_geodetic(17.8850833, 28.7572778, hdr['TELALT'])
    mjd = Time(hdr['DATE-AVG'], format='isot', scale='utc').mjd
    t = Time(mjd, format='mjd',scale='utc', location=location)

    tdbholder= (utc_tdb.JDUTC_to_BJDTDB(t, ra=float(pointing.ra.degree), dec=float(pointing.dec.degree), lat=28.7572778, longi=17.8850833, alt=hdr['TELALT'], leap_update=True))

    return (tdbholder[0][0]-2400000.5)


def gather_files(paths, filelist=None, filetype="fz", bjd=False, ignoreedgefraction=0.0, lowest=0):
    # Get list of files
    ...
    if not filelist:
        if filetype not in ['fits', 'fit', 'fz']:
            ...
            filelist = paths['parent'].glob("*.{}".format(filetype)) # Make sure only fully reduced LCO files are used.
    if filetype not in ['fits', 'fit', 'fz']:
...
def find_stars(targets, paths, fileList, photCoords=None, photFileHolder=None, mincompstars=0.1, mincompstarstotal=-99, starreject=0.1 , acceptDistance=1.0, lowcounts=0, hicounts=3000000, imageFracReject=0.0,  rejectStart=3, maxcandidatestars=10000):
    """
