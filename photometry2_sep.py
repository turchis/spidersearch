...
def photometry(image,image_file,threshold,min_area,star_scale,cfwhm,min_fwhm,max_fwhm,analysis):
           ...
           # print(len(sources))
            ...
            #print(len(sources))
            #sources['targfwhm'] = 2 * np.sqrt(np.log(2) * (sources['x2'] + sources['y2']))
            sources = sources[sources['targfwhm'] <= max_fwhm]
            sources = sources[sources['targfwhm'] >= min_fwhm]
            #print(len(sources))
            #sources['targfwhm'] = np.nan
            #sources['fwtm'] = np.nan
            ...
            #                        source['targfwhm'] = 2.0 * np.nanmax(contour_radii)
            ...
            catalog = sources['x', 'y', 'xwin', 'ywin', 'xpeak', 'ypeak',
                              'flux', 'fluxerr', 'peak', 'background', 'targfwhm',
                              'fwhm', 'a', 'b', 'theta', 'aprad', 'ellipticity',
...
#image_file = get_pkg_data_filename(path+'/cpt1m012-fa06-20220727-0066-e91.fits')  
#image = fits.open(path+'/cpt1m012-fa06-20220727-0066-e91.fits')
#photometry(image,image_file=image_file,threshold=37.0,min_area=2,star_scale=1.5,max_fwhm=16)
#print('cpt1m012-fa06-20220727-0066-e91.fits')
#image.close()
...
#zimages = np.genfromtxt(path+'/zfilter.txt',dtype='str')
#zimages = zimages.tolist()
#nz = len(np.genfromtxt(path+'/zfilter/usedImages.txt',dtype='str'))-1
#zcomps = np.atleast_2d(np.genfromtxt(path+'/zfilter/compsUsed.csv', delimiter=','))

#getting fwhm of the comparison stars for each frame and average between them

if np.shape(gcomps)[0]>1:
            fwhm = np.zeros((np.shape(gcomps)[0],ng))
            for i in range(0,np.shape(gcomps)[0]):
...
if np.shape(rcomps)[0]>1:
            fwhm = np.zeros((np.shape(rcomps)[0],nr))
            for i in range(0,np.shape(rcomps)[0]):
...
if np.shape(icomps)[0]>1:
            fwhm = np.zeros((np.shape(icomps)[0],ni))
            for i in range(0,np.shape(icomps)[0]):
...
#if np.shape(zcomps)[0]>1:
#            fwhm = np.zeros((len(zcomps),nz))
#            for i in range(0,len(zcomps)):
#                        cfwhm = np.genfromtxt(path+'/zfilter/outputcats/doerPhot_V'+str(i+1)+'.csv', delimiter=',',usecols=(6),unpack=True)
#                        for j in range(0,len(cfwhm)):
#                                    fwhm[i,j] = cfwhm[j]
#            zfwhm_avg = np.zeros(nz)
#            for i in range(0,nz):
#                        zfwhm_avg[i] = np.mean(fwhm[:,i])
...
if np.shape(gcomps)[0]>1:
            ...
            photometry(image,image_file=image_file,threshold=2,min_area=2,star_scale=fext,cfwhm=gfwhm,min_fwhm=2,max_fwhm=10,analysis=analysis)
            print('median_g.fits')
            image.close()
            for i in range(0,(len(gimages))):
                        ...
                        photometry(image,image_file=image_file,threshold=s,min_area=2,star_scale=fext,cfwhm=gfwhm_avg[min([i,ng-1])],min_fwhm=2,max_fwhm=10,analysis=analysis)
                        print(gimages[i])
                        image.close()
...
if np.shape(rcomps)[0]>1:
            ...
            photometry(image,image_file=image_file,threshold=2,min_area=2,star_scale=fext,cfwhm=rfwhm,min_fwhm=2,max_fwhm=10,analysis=analysis)
            print('median_r.fits')
            image.close()
            for i in range(0,(len(rimages))):
                        ...
                        photometry(image,image_file=image_file,threshold=s,min_area=2,star_scale=fext,cfwhm=rfwhm_avg[min([i,nr-1])],min_fwhm=2,max_fwhm=10,analysis=analysis)
                        print(rimages[i])
                        image.close()
                        print("image should now be closed")
...
if np.shape(icomps)[0]>1:
            ...
            photometry(image,image_file=image_file,threshold=2,min_area=2,star_scale=fext,cfwhm=ifwhm,min_fwhm=2,max_fwhm=10,analysis=analysis)
            print('median_i.fits')
            image.close()
            for i in range(0,(len(iimages))):
                        ...
                        photometry(image,image_file=image_file,threshold=s,min_area=2,star_scale=fext,cfwhm=ifwhm_avg[min([i,ni-1])],min_fwhm=2,max_fwhm=10,analysis=analysis)
                        print(iimages[i])
                        image.close()
                        print("image should now be closed")

#if np.shape(zcomps)[0]>1:
#            image_file = path+'/zfilter/median_z.fits'  
#            image = fits.open(path+'/zfilter/median_z.fits')
#            zfwhm = np.mean(zfwhm_avg)
#            photometry(image,image_file=image_file,threshold=2,min_area=2,star_scale=fext,cfwhm=zfwhm,min_fwhm=3,max_fwhm=10,analysis=analysis)
#            print('median_z.fits')
#            image.close()
#            for i in range(0,(len(zimages))):
#                        image_file = path+'/zfilter/'+zimages[i]  
#                        image = fits.open(path+'/zfilter/'+zimages[i])
#                        if zfwhm_avg[min([i,nz-1])]==0.:
#                                    zfwhm_avg[min([i,nz-1])] = zfwhm
#                        photometry(image,image_file=image_file,threshold=s,min_area=2,star_scale=fext,cfwhm=zfwhm_avg[min([i,nz-1])],min_fwhm=3,max_fwhm=10,analysis=analysis)
#                        print(zimages[i])
#                        image.close()
#                        print("image should now be closed")
